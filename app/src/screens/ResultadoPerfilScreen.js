    ...
    <TouchableOpacity style={styles.card} onPress={() => onPress(item)} activeOpacity={0.85}>
      ...
      <View style={styles.cardBody}>
        ...
        <View style={styles.cardFooter}>
          ...
          {dias !== null && dias !== undefined && dias <= 15 && (
...
export default function ResultadoPerfilScreen({ navigation }) {
  ...
  async function buscarConcursos() {
    try {
      ...
      if (answers.area)   query = query.eq('area', answers.area);
      if (answers.estado) query = query.or(`estado.eq.${answers.estado},estado.eq.FEDERAL`);
  ...
  function irParaConvocatoria(item) {
    ...
    setTimeout(() => navigation.navigate('Convocatoria', { convocatoria: item }), 300);
  }
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <Text style={styles.headerTitulo}>
          {loading ? 'Buscando plazas...' : `${concursos.length} plazas encontradas`}
        </Text>
        <Text style={styles.headerSub}>
          Perfil: {areaLabel} · {estadoNome}
        </Text>
      </LinearGradient>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ConvocatoriaCard item={item} onPress={irParaConvocatoria} />
          )}
